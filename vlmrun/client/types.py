...
class StreamEvent(BaseModel):
    ...
    type: StreamEventType = Field(
        ...,
        description="Type of the stream event: 'status' for status updates, 'heartbeat' for keep-alive, 'result' for final response, 'error' for errors.",
    )
    ...
    elapsed_seconds: Optional[float] = Field(
        None, description="Time elapsed since request started in seconds."
    )
    response: Optional[Any] = Field(
        None, description="The response data (only present in 'result' events)."
    )
    usage: Optional[CreditUsage] = Field(
        None, description="Usage metrics (only present in 'result' events)."
    )
    ...
    completed_at: Optional[datetime] = Field(
        None,
        description="When the request completed (only present in 'result' events).",
    )
